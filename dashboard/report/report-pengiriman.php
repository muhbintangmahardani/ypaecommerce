...
// Show Data
$query = "SELECT * FROM `tb_cart`, `tb_detail_order`, `tb_produk` WHERE `tb_produk`.`id_product` = `tb_detail_order`.`id_product` AND `tb_cart`.`id_order` = `tb_detail_order`.`id_order` AND `tb_cart`.`status` != 'cart' ORDER BY `tb_detail_order`.`id_detail` ASC";
...
<body>
    <h2 align="center">Laporan Pengiriman</h2>
...
$mpdf->Output('report-pengiriman.pdf', 'D');
?>